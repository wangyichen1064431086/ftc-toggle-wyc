...
gulp.task('html',() => {
  	...
  	return co(function *(){
	    const destDir = '.tmp';
	    if(!isThere(destDir)){//如果../ft-interact/demos不存在存在，则生成这个目录
	      mkdirp(destDir,(err) => {
	        if(err){
	           console.log(err);
	        }
        ...
        */
	    ...
	    const renderResults = yield Promise.all(demos.map(function(demo){
	    	...
	    	*/
	    	...
	    	const context = {
	    		pageTitle:demo.name,//"toggles"
	    		description:demo.description,//""
	    		embeded:embeded//dev环境下是false
	    	};

	    	return helper.render(template,context,demo.name);//模板是template，数据是context,demo.name为toggles
	    ...
	    yield Promise.all(renderResults.map(result => {
	    	...
	    	*/
    		...
    		const dest = `.tmp/${result.name}.html`;//".tmp/toggles.html"
